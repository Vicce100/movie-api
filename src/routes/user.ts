...
import { CurrentUserType, UserType } from '../utilities/types.js';
...
router.post('/create', async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json("Canno't Find User");
...
router.post('/refreshToken', async (req, res) => {
  ...
  const usern: UserType | null = await User.findOne({ refreshToken });
  if (usern === null) return res.sendStatus(401); // check if refreshToken exist in data base
  if (process.env.SECRET_REFRESH_TOKEN)
...
router.delete('/logout', authenticateToken, (req: any, res) => {
  User.updateOne(
...
// deleting one User
router.delete('/delete', authenticateToken, async (req: any, res) => {
  const { username, password, userId } = req.body;
  if (req.user.username === username) {
    const usern: UserType | null = await User.findOne({ username });
    if (usern === null) return res.status(404).json('Cannot find user');
